...
namespace IotSensorMon
{
    public partial class FrmMain : Form
    {
        ...
        private bool IsSimulation = false; //시뮬레이션 물어봄
        ...
        private void InitCharStyle()
        {
            ...
            ChtPhotoResistors.ChartAreas[0].AxisX.ScaleView.Zoomable = true; //마우스 스크롤로 줌인 줌아웃 함
            ...
            ChtPhotoResistors.Series.Clear(); //series 값 지움
        ...
        private long timeSpan = 0; //시간흐름의 값
        ...
        private void ShowSensorValue(string value)
        {
            ...
            //리스트박스에 계속 데이터 출력
            var item = $"{currentTime.ToString("yyyy-MM-dd HH:mm:ss")}\t{v}";// \t 간격을 띄우기
            LsbPhotoResistor.Items.Add(item);
            LsbPhotoResistor.SelectedIndex = LsbPhotoResistor.Items.Count - 1; //스크롤 처리, count -1은  배열을 말한다 0~10까지는 9개인데 보여지는것은 배열설정에서 하나를 뺀값
            ...
            //200이상이 되었을때 zoom했을때 xcount의 뺀값 범위 까지 나오게 한다
            else
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connString))
                {
                    ...
                    var query = $"insert into Tbl_PhotoResistor " +
                        ...
                        $" values " +
                        $"('{data.Current.ToString("yyyy-MM-dd HH:mm:ss")}', '{data.Value}', '{(data.SimulFlag == true ? "1" : "0")}')";//1은 true 0은 아님
        ...
        private void MnuExit_Click(object sender, EventArgs e)
        {
            if(IsSimulation)
            {
                MessageBox.Show("시뮬레이션 멈춘후 프로그램을 종료하세료", "종료",
