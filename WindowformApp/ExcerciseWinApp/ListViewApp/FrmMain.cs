...
namespace ListViewApp
{
    public partial class FrmMain : Form
    {
        ...
        {
            ListViewItem itemSwitch = new ListViewItem("Ninntendo Switch", 0);
            ...
            ListViewItem itemPs = new ListViewItem("PlayStation", 2);
        ...
        private void LsvProducts_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            foreach (ListViewItem item in Selected)
            {
                for (int i = 0; i < 4; i++)
                {
                    TxtSelected.Text += item.SubItems[i].Text + " "; //각 아이템을 돌면서 서브아이템을 출력한다
                }
            }
        }
    }
